...
class TestJob < Minitest::Test
  ...
  def test_initialize
    job = Zhong::Job.new("test", {at: "12:00"}.merge(default_config))

    assert job
    assert !job.running?
    assert_equal "test", job.to_s
  end

  def test_should_run
    sleep 1

    job = Zhong::Job.new("test", {every: 1.second}.merge(default_config))
  ...
  def test_run
    ...
    job = Zhong::Job.new("test", {every: 1.second}.merge(default_config)) { success_counter << 1 }
